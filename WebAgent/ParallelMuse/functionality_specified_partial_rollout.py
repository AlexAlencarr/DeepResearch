    ...
    async with sem:
        ...
        for tok, toplogprobs in zip(result_tokens, result_toplogprobs):
            logprob_values = np.array(
                [tlp.logprob for tlp in toplogprobs], dtype=np.float64
            )
        ...
        if "<tool_call>" in "".join(result_tokens):
            ...
            think_ppl = (
                np.exp(np.mean(entropies[think_start + 1 : think_end]))
                if think_end
                else -1
            )
            tool_call_ppl = (
                np.exp(np.mean(entropies[tool_call_start + 1 : tool_call_end]))
                if tool_call_start and tool_call_end
                else -1
            )
            ...
            step_ppl = {"think_ppl": -1, "tool_call_ppl": -1, "all_ppl": -1}
...
def get_initial_rollouts(question, existing_rollouts, initial_rollout_num):
    ...
    if len(initial_rollouts) != initial_rollout_num:
        for rollout in all_rollouts:
            if (
                rollout not in initial_rollouts
                and rollout["termination"] != "max_length_exceeded"
                and rollout["termination"] != "llm_error_occurred"
            ):
                initial_rollouts.append(rollout)
            if len(initial_rollouts) == initial_rollout_num:
                break
...
async def rollout_single_traj(
    ...
    args,
    max_turn_given=None,
    rollout_type="traj_level_rollout",
):
    ...
    for turn in range(max_turn):
        ...
        if llm_response is None:
            return {
                ...
                "answer": answer,
                "rollout": record,
                "termination": "llm_error_occurred",
        ...
        record.append(
            {
                ...
                "previous_turn_time_consumption": previous_turn_time_consumption,
                "step_ppl": step_ppl,
            }
        )

        if "<tool_call>" in llm_response and "</tool_call>" in llm_response:
            tool_call_str = llm_response.split("<tool_call>")[-1].split("</tool_call>")[
                0
            ]
            ...
            try:
                ...
                print(
                    f"Tool call {tool_name} invocation success with length {len(tool_response)}"
                )
                print(tool_response)
            except Exception as e:
...
def branch_high_uncertainty_steps(
    rollout, partial_sampling_topk, partial_sampling_mode
):
    branch_step = []
    ...
    if partial_sampling_mode != "mixed_ppl":
        for i, msg in enumerate(rollout):
            if msg.get("step_ppl", None):
                branch_step.append(
                    {"step_id": i, "step_ppl": msg["step_ppl"][partial_sampling_mode]}
                )

        branch_step = sorted(branch_step, key=lambda x: x["step_ppl"], reverse=True)[
            :partial_sampling_topk
        ]
        ...
        for i, msg in enumerate(rollout):
            if msg.get("step_ppl", None):
                tool_call_branch_step.append(
                    {"step_id": i, "step_ppl": msg["step_ppl"]["tool_call_ppl"]}
                )

        tool_call_branch_step = sorted(
            tool_call_branch_step, key=lambda x: x["step_ppl"], reverse=True
        )[:tool_call_sampling_topk]

        for i, msg in enumerate(rollout):
            if msg.get("step_ppl", None):
                think_branch_step.append(
                    {"step_id": i, "step_ppl": msg["step_ppl"]["think_ppl"]}
                )

        think_branch_step = sorted(
            think_branch_step, key=lambda x: x["step_ppl"], reverse=True
        )[:think_sampling_topk]
...
async def main(args):
    ...
    full_traj_rollout_output_file_path = os.path.join(
        args.output_dir,
        f"{args.qa_file_path.split('/')[-1].replace('.jsonl', '')}_{args.model_path.split('/')[-1]}_1_none_initial_rollout.jsonl",
    )
    if args.partial_sampling_mode == "none":
        ...
        output_file_path = os.path.join(
            args.output_dir,
            f"{args.qa_file_path.split('/')[-1].replace('.jsonl', '')}_{args.model_path.split('/')[-1]}_{args.initial_rollout_num}_{args.partial_sampling_mode}_{args.partial_sampling_topk}_{args.partial_sampling_rounds}_{args.partial_sampling_times_per_pos}.jsonl",
        )
    ...
    if (
        ...
        visited_initial_rollouts = visited_rollouts[
            : initial_num * args.initial_rollout_num
        ]
        ...
        fully_visited_question = [
            question
            for question, count in visited_rollouts_counter.items()
            if count == args.sampling_budget - args.initial_rollout_num
        ]
        fully_visited_rollouts = [
            rollout
            for rollout in visited_rollouts
            if rollout["question"] in fully_visited_question
        ]
    ...
    if args.partial_sampling_mode == "none":  # traj-level rollout
        ...
        for data in dataset:
            ...
            for _ in range(need_to_submit):
                ...
                tasks.append(
                    rollout_single_traj(llm_sem, tool_sem, data, messages, args)
                )
        ...
        assert (
            ...
            + initial_rollout_num
            * partial_sampling_topk
            * partial_sampling_rounds
            * partial_sampling_times_per_pos
        ), "Sampling budget is not set correctly."
        try:
            assert all(
                visited_counter[data["question"]] >= args.initial_rollout_num
                for data in dataset
            ), "Initial rollouts are not sufficient."
        except:
            raise ValueError

        for i, data in tqdm(
            enumerate(dataset), total=len(dataset), desc="Detecting branching point ..."
        ):
            question = data["question"]
            ...
            if not visited_initial_rollouts:
                initial_rollouts = get_initial_rollouts(
                ...
                initial_rollouts = [
                    v_initial_rollouts
                    for v_initial_rollouts in visited_initial_rollouts
                    if v_initial_rollouts["question"] == question
                ]

            assert (
                len(initial_rollouts) == initial_rollout_num
            ), "Initial rollouts are not sufficient."

            for sampling_round in range(
                partial_sampling_rounds
            ):  # TODO: multi-round partial rollout, now only support one round
                for r in initial_rollouts:
                    tmp_tasks_count = 1
                    ...
                    sampling_budget_per_initial_rollout = int(
                        sampling_budget / initial_rollout_num
                    )
                    if (
                        sampling_budget_per_initial_rollout
                        > 1
                        + partial_sampling_topk
                        * partial_sampling_rounds
                        * partial_sampling_times_per_pos
                    ...
                    assert (
                        tmp_tasks_count == sampling_budget_per_initial_rollout
                    ), "Sampling budget mismatch."
...
if __name__ == "__main__":
    ...
    # basic info
    parser.add_argument(
        "--qa_file_path", type=str, default="./data/browsecomp_200.jsonl"
    )
    parser.add_argument("--output_dir", type=str, default="./deepresearch_results")
    parser.add_argument(
        "--model_path", type=str, default="./llm_ckpt/tongyi-deepresearch-30b-a3b"
    )
