...
def o1_api(ds, output_path):
    ...
    async def get_chat_completion(message, semaphore) -> str:
        try:
            ...
            print(
                f"Error in get_chat_completion for message  {type(e).__name__} - {str(e)}"
            )
            raise
    ...
    async def request_model(prompts):
        ...
        for future in tqdm.as_completed(
            tasks, total=len(tasks), desc="Processing prompts"
        ):
            result = await future
...
def gemini_api(ds, output_path, search=False):
    ...
    @retry(
        stop=stop_after_attempt(10), wait=wait_exponential(min=4, max=60), reraise=True
    )
    ...
    async def run_gemini_api():
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for future in tqdm.as_completed(
                tasks, total=len(tasks), desc="Processing queries"
            ):
                query_text, result = await future
                adic = {
                    "question": query_text,
                    "pred": result["candidates"][0]["content"]["parts"][0]["text"],
                }
                with open(output_path, "a", encoding="utf-8") as f:
...
def kimi_api(ds, output_path):
    ...
    with tqdm(total=len(data_list)) as pbar:
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            future_to_data = {
                executor.submit(process_data, data): data for data in data_list
            }
            for future in concurrent.futures.as_completed(future_to_data):
    ...
    if api_name == "all":
        for api in api_functions:
            ...
            if api != "gemini_search_api":
                ...
                api_functions[api](
                    ds, output_path + "/" + api + "_result.jsonl", search=True
                )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        help="Path to the output file. If api_name is 'all', this should be a directory. If api_name is others, this should be a file.",
    )
