...
if "DASHSCOPE_API_KEY" not in os.environ and "OPENAI_API_KEY" not in os.environ:
    raise ValueError(
        "Please set the environment variable 'DASHSCOPE_API_KEY' or 'OPENAI_API_KEY' to your API key."
    )
...
def extract_links_with_text(html):
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if (
            ...
            and not url.endswith((".jpg", ".png", ".gif", ".jpeg", ".pdf"))
        ):
...
if __name__ == "__main__":
    ...
    st.markdown(
        "👋Welcome to WebWalker! WebWalker is a web-based conversational agent that can help you navigate websites and find information."
    )
    st.markdown("📑The paper of WebWalker is available at [arXiv]().")
    st.markdown(
        "✨You can bulid your own WebWalker by following the [instruction](https://github.com/Alibaba-NLP/WebWalker)."
    )
    st.markdown(
    ...
    with st.sidebar:
        MAX_ROUNDS = st.number_input(
            "Max Count Count：", min_value=1, max_value=15, value=10, step=1
        )
        website_example = st.sidebar.selectbox(
            "Example Website：", ["https://2025.aclweb.org/"]
        )
        question_example = st.sidebar.selectbox(
            "Example Query：",
            [
                "When is the paper submission deadline for ACL 2025 Industry Track, and what is the venue specific address?",
                "Who is the general chair of ACL 2025?",
                "What is the spcial theme of ACL 2025?",
            ],
        )
    ...
    with col1:
        with st.form(key="my_form"):
            ...
            website = st.text_area(
                "👉Website",
                value=website_example,
                placeholder="Input the website you want to walk through.",
            )
            query = st.text_area(
                "🤔Query",
                value=question_example,
                placeholder="Input the query you want to ask.",
            )
                ...
                if website and query:
                    ...
                    with col2:
                        ...
                        if screenshot:
                            ...
                            with open(
                                image_folder
                                + str(st.session_state.image_index)
                                + ".png",
                                "wb",
                            ) as f:
                            ...
                            image_path = os.path.join(
                                image_folder, image_files[st.session_state.image_index]
                            )
                            image = Image.open(
                                image_folder
                                + str(st.session_state.image_index)
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        if "button" in json5.loads(params):
            ...
            if json5.loads(params)["button"].replace("<button>", "") in BUTTON_URL_ADIC:
                ...
                url = BUTTON_URL_ADIC[
                    json5.loads(params)["button"].replace("<button>", "")
                ]
                ...
                with col2:
                    ...
                    if screenshot:
                        ...
                        with open(
                            image_folder
                            + str(st.session_state.image_index + 1)
                            + ".png",
                            "wb",
                        ) as f:
                            f.write(base64.b64decode(screenshot))
                        ...
                        image = Image.open(
                            image_folder + str(st.session_state.image_index) + ".png"
                        )
                        st.image(
                            image,
                            caption="Step "
                ...
                response += (
                    "Clickable buttons are wrapped in <button> tag" + response_bottons
                )
                return response
            else:
