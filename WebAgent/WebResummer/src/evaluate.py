...
def process_one_prediction(prediction_file):
    try:
        ...
        if os.path.exists(scored_file):
            print(
                f"Found existing scored file for {iteration_name}, loading results..."
            )
            ...
            for scored_item in scored_items:
                ...
                score_dict[scored_item["question"]] = scored_item.get(
                    "is_correct", False
                )

            acc = round(len(correct_predictions) / len(scored_items) * 100, 2)
            print(
                f"Loaded scored file: {scored_file} has {len(correct_predictions)} correct predictions (total {len(scored_items)}). Pass@1 {acc}%"
            )
        ...
        print(
            f"Prediction file: {prediction_file} has {len(correct_predictions)} correct predictions (total {len(predictions)}). Pass@1 {acc}%"
        )
                ...
                for judgement_result in judgement_results:
                    orig_item = next(
                        (
                            ...
                            if item["question"] == judgement_result["question"]
                        ),
                        None,
                    )
        ...
        return {"file": prediction_file, "error": str(e)}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--restore_result_path", default="output/summary.jsonl", help="record result"
    )
    args = parser.parse_args()

    available_tools = (
        args.available_tools.split(",") if args.available_tools else ["search", "visit"]
    )
    ...
    if all_stats:
        ...
        for key in all_stats[0].keys():
            avg_stats[key] = round(
                sum(stats.get(key, 0) for stats in all_stats) / len(all_stats), 2
            )
