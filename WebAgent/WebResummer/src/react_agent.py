...
class MultiTurnReactAgent(FnCallAgent):
    ...
    def _run(
        self, data: str, model: str, summary_iteration: int, **kwargs
    ) -> List[List[Message]]:
        self.model = model
        ...
        while num_llm_calls_available > 0:
            ...
            if "<tool_call>" in content and "</tool_call>" in content:
                ...
                try:
                    ...
                    print(
                        f"Tool call {tool_name} invocation success with length {len(result)}"
                    )
                except Exception as e:
            ...
            if should_summarize:
                ...
                try:
                    summary_response = summarize_conversation(
                        question, recent_messages, last_summary
                    )
                    print(
                        f"[Summary Tool] ReSum-Tool-30B Invocation success (len: {len(summary_response)}): {summary_response}"
                    )
                except Exception as e:
            ...
            if token_count > max_tokens:
                ...
                messages[-1][
                    "content"
                ] = "You have now reached the maximum context length you can handle. You should stop invoking tools and, based on all the information above, think again and provide what you consider the most likely answer in the following format: <think> your final thinking </think>\n <answer> your answer </answer>"
                content = self.call_server(messages)
