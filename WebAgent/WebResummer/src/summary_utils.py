    ...
    for _ in range(max_retries):
        try:
            response = requests.post(
                RESUM_TOOL_URL, headers=header, json=data, timeout=360
            )
            ...
            if _ == max_retries - 1:
                print(
                    f"[Summary Tool] Server attempt {_+1}/{max_retries} failed: {str(e)}"
                )
                return ""
    return ""
...
def summarize_conversation(
    question, recent_history_messages, last_summary, max_retries=10
):
    recent_history_str = "\n".join([str(msg) for msg in recent_history_messages])

    if not last_summary:
        query_prompt = QUERY_SUMMARY_PROMPT.replace("{{{question}}}", question).replace(
            "{{{recent_history_messages}}}", recent_history_str
        )
    else:
        query_prompt = (
            QUERY_SUMMARY_PROMPT_LAST.replace("{{{question}}}", question)
