        ...
        for message in messages:
            if message.role == USER:
                message.content[0].text = (
                    self.custom_user_prompt + message.content[0].text
                )
                break
        return messages
        ...
        **kwargs,
        ...
        if self.make_system_prompt:
            if not messages or messages[0][ROLE] != SYSTEM:
                messages.insert(
                    0, Message(role=SYSTEM, content=self.make_system_prompt())
                )
        for msg in messages:
        ...
        while True and num_llm_calls_available > 0:
            ...
            if output:
                ...
                for out in output:
                    ...
                    logger.info(
                        f"{self.name} use_tool: {use_tool}, tool_name: {tool_name}, tool_args: {tool_args}"
                    )
                    if use_tool:
