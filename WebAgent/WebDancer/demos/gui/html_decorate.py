...
def decorate_writing(writing_result, style="Default"):
    ...
    def replace_cite(match):
        ...
        for url in urls:
            ...
            citation_html.append(
                (
                    f'<a href="{url}" title="点击查看引用来源: {url}">{current_index}</a>',
                    current_index,
                )
            )
    ...
    decorated_result = re.sub(
        takeaway_pattern, replace_takeaway, decorated_result, flags=re.S
    )
...
"""
    ...
    decorated_result = re.sub(
        mermaid_pattern, decorate_mermaid, decorated_result, flags=re.S
    )
...
<script>
    ...
    decorated_result = re.sub(
        echarts_pattern, replace_echarts, decorated_result, flags=re.S
    )
