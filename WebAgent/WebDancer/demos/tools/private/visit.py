            ...
            if (
                ...
                try:
                    ...
                    useful_information += (
                        "Evidence in page: \n"
                        + rawjson.get(
                            "evidence", "The provided webpage content is not in json."
                        )
                        + "\n\n"
                    )
                    useful_information += (
                        "Summary: \n"
                        + rawjson.get(
                            "summary", "The webpage content is not processed in json"
                        )
                        + "\n\n"
                    )
                    ...
                    raw = (
                        self.llm(messages).replace("```json\n", "").replace("\n```", "")
                    )
                    parse_retry_times += 1
...
if __name__ == "__main__":
    print(
        Visit().readpage(
            "https://github.com/callanwu/WebWalker-1?tab=readme-ov-file", "who are you?"
        )
    )
