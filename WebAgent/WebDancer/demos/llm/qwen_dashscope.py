        ...
        if response.status_code == HTTPStatus.OK:
            return [
                Message(
                    ...
                    reasoning_content=response.output.choices[0].message.get(
                        "reasoning_content", ""
                    ),
        ...
        for chunk in response:
            if chunk.status_code == HTTPStatus.OK:
                if chunk.output.choices[0].message.get("reasoning_content", ""):
                    full_reasoning_content += chunk.output.choices[
                        0
                    ].message.reasoning_content
...
def initialize_dashscope(cfg: Optional[Dict] = None) -> None:
    ...
    if api_key in ("", "EMPTY"):
        if dashscope.api_key is None or dashscope.api_key in ("", "EMPTY"):
            ...
            logger.info(
                "No dashscope api_key found in cfg, using the dashscope.api_key that has already been set."
            )
    else:  # valid api_key
