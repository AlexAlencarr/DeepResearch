...
from prompt import JUDGE_PROMPT_BC, JUDGE_PROMPT_GAIA, JUDGE_PROMPT_QA
...
def call_llm_judge(item):
    ...
    try:
        ...
        return {"question": question, "answer": correct_answer, "judgement": response}
...
def single_round_statistics(input_file):
    ...
    return {
        ...
        "avg_action": sum(tool_use_cnt)
        / len(tool_use_cnt),  # avg. number of tool invocation
        "avg_visit_action": sum(visit_tool_cnt)
        / len(visit_tool_cnt),  # avg. number of visit tool invocation
        "avg_search_action": sum(search_tool_cnt)
        / len(search_tool_cnt),  # avg. number of search tool invocation
        "avg_other_action": sum(other_tool_cnt)
        / len(other_tool_cnt),  # avg. number of other tool invocation
...
def main():
    ...
    if dataset in ["gaia", "webwalker", "xbench-deepsearch", "hle"]:
        ...
        judge_prompt = JUDGE_PROMPT_BC
    elif dataset.endswith("qa") or dataset.endswith(
        "qa_small"
    ):  # for simple_qa and time_qa
    ...
    round_performance = {
        f"Round{i}_Pass@1": round(
            sum(1 for r in round_results[f"round{i}"] if r["judgement"] == "Correct")
