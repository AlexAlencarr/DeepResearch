...
from prompts import *
from toolkit.browser import Click, Fill, Visit
from toolkit.mcp_client import mcp_client
from toolkit.tool_search import Search
from tqdm import tqdm
from transformers import AutoTokenizer
from utils import call_llm, count_tokens, read_jsonl
...
async def call_tool(sem, tool_name: str, tool_args: dict, client, lock):
    async with sem["tool"]:
        if tool_name == "search":
            ...
            return await visit.call(
                tool_args, client=client, lock=lock, tokenizer=tokenizer, sem=sem
            )
        elif tool_name == "click":
            return await click.call(
                tool_args, client=client, lock=lock, tokenizer=tokenizer, sem=sem
            )
        elif tool_name == "fill":
                ...
                if "<tool_call>" in response and "</tool_call>" in response:
                    ...
                    try:
                        ...
                        result = await call_tool(
                            sem, tool_name, tool_args, client, lock
                        )
                        ...
                        print(
                            f"Tool call {tool_name} invocation success with length {len(observation)}"
                        )
                        print(observation)
...
async def main(sem, rollout_count, input_path, output_path):
    ...
    for data in dataset:
        ...
        need_to_submit = (
            rollout_count - total_count if rollout_count - total_count > 0 else 0
        )
