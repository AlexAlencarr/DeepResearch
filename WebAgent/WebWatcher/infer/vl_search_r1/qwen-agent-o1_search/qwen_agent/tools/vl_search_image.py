...
enable_search_cache = os.getenv("VL_IMG_SEARCH_ENABLE_CACHE", "false").lower() in (
    "y",
    "yes",
    "t",
    "true",
...
cache = JSONLCache(
    os.path.join(os.path.dirname(__file__), "vl_search/search_cache_image.jsonl")
)
...
class VLSearchImage(BaseTool):
    ...
    def try_download(self, url, method, timeout, max_retries):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if method == "curl":
                    ...
                    try:
                        ...
                        if (
                            ...
                            and response_json.get("data").get("type") == "b64"
                        ):
                ...
                logging.warning(
                    f"{method.capitalize()} failed for {url} on attempt {attempt + 1}/{max_retries}: {e}"
                )
        return None
    ...
    def download_upload(
        self, search_data, img_save_path, byte, max_retries=5, max_time=10
    ):
        ...
        for item in tqdm(search_data, desc="download"):
            ...
            if image_data is None:
                ...
                image_data = self.try_download(
                    image_path, "wget", max_time, max_retries
                )

                if image_data is None:
                    logging.error(
                        f"Using request to download {image_path} (curl and wget failed)"
                    )
                    image_data = self.try_download(
                        image_path, "request", max_time, max_retries
                    )
    ...
    def parse_image_search_result(self, search_result):
        ...
        return (search_images, search_texts, search_urls)
    ...
    def call(self, items, messages=None, img_save_path=None, byte=False, **kwargs):
        ...
        for image_url in image_urls:
            search_result = self.search_image_by_image_url(
                image_url, img_save_path, byte
            )

            search_images, search_texts, search_urls = self.parse_image_search_result(
                search_result
            )
...
if __name__ == "__main__":
    ...
    image_url1 = [
        "https://mitalinlp.oss-cn-hangzhou.aliyuncs.com/rallm/deep_research_vl_image/hle_image/1.jpg"
    ]
    image_url2 = [
        "https://th.bing.com/th/id/OIP.sCzpnScidvdeSPyy-0Rd2wHaDt?o=7rm=3&rs=1&pid=ImgDetMain&o=7&rm=3"
    ]

    results = VLSearchImage().call(
        {
            "images": [
                "https://mitalinlp.oss-cn-hangzhou.aliyuncs.com/rallm/deep_research_vl_image/hle_image/290.jpg"
            ]
        }
    )
    print(results)
