...
enable_search_cache = os.getenv("VL_TEXT_SEARCH_ENABLE_CACHE", "false").lower() in (
    "y",
    "yes",
    "t",
    "true",
...
cache = JSONLCache(
    os.path.join(os.path.dirname(__file__), "vl_search/search_cache_text.jsonl")
)
...
class VLSearchText(BaseTool):
    ...
    def search_image_by_text(self, gold_query, max_retry: int = 10, timeout: int = 30):
        ...
        headers = {
            ...
            "Authorization": f"Bearer {TEXT_SEARCH_KEY}",
        ...
        for _ in range(max_retry):
            try:
                resp = requests.post(
                    url, headers=headers, data=json.dumps(template), timeout=timeout
                )
    ...
    def upload(self, image, byte=False):
        ...
        target_path = (
            f"rallm/deep_research_vl_image/hle_eval_image_gpt4o_25_0325_v2/{image_name}"
        )
