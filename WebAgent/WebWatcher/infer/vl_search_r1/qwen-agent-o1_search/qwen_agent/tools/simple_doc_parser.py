...
from qwen_agent.utils.str_processing import (
    rm_cid,
...
def parse_file_by_idp(
    ...
    cred_config = CredentialProviderConfig(
    ...
    for x in result["layouts"]:
        if pages and pages[-1]["page_num"] == x["pageNum"][0]:
            ...
            pages.append(
                {"page_num": x["pageNum"][0], "content": [{"text": x["text"]}]}
            )
...
class SimpleDocParser(BaseTool):
    description = (
        f'提取出一个文档的内容，支持类型包括：{"/".join(PARSER_SUPPORTED_FILE_TYPES)}'
    )
    parameters = [
        {
            "name": "url",
    ...
    def call(self, params: Union[str, dict], **kwargs) -> Union[str, list]:
        ...
        try:
            ...
            if USE_IDP:
                try:
                    ...
                    if path.startswith("https://") or path.startswith("http://"):
                        ...
                        parsed_file = parse_file_by_idp(
                            file_path=path, file_type=f_type
                        )
            ...
            time2 = time.time()
            logger.info(
                f"Finished parsing {path}. Time spent: {time2 - time1} seconds."
            )
            # Cache the parsing doc
            self.db.put(
                cached_name_ori, json.dumps(parsed_file, ensure_ascii=False, indent=2)
            )
