        ...
        **kwargs,
        ...
        if not safe:
            if "Banned Error" in message:
                print(
                    f"[code_interpreter_error] banned operation found, skip the following code\n{code}"
                )
            return message
        ...
        if not safe:
            print(
                f"[code_interpreter_error] unsafe file operation found, skip the following code\n{code}"
            )
            return message
        ...
        result = execute_with_timeout(
            self._execute_code, timeout, kc=kc, code=fixed_code
        )
    ...
    def _execute_code(self, kc, code: str) -> str:
        ...
        while True:
            ...
            try:
                ...
                print(f"[FATAL] An exception occurred: {e}", flush=True)
                text = (
                    "Fatal Error: The code interpreter encountered an unexpected error."
                )
                print_traceback()
                finished = True
            if text:
...
def _check_deps_for_code_interpreter():
    try:
        ...
        raise ImportError(
            ...
            'Please install the required dependencies by running: pip install "qwen-agent[code_interpreter]"'
        ) from e
