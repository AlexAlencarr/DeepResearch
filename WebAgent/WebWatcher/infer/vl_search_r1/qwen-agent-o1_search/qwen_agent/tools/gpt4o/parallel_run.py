...
from utils import APIException, get_args, load_file2list
...
def update_call_args(data, call_args):
    ...
    for key in remove_keys:
        call_args.pop(key)
    assert not (
        "messages" in data.get("call_args", {}) and data.get("prompt") not in [None, ""]
    )
    system = None
...
def llm_api_generate(args, api_call):
    ...
    with open(out_file, write_mode, encoding="utf-8") as writer, ThreadPoolExecutor(
        args.num_workers
    ) as executor:
        Logger.info(
            f"gen answer uuids: {' ,'.join([str(d['uuid']) for d in oai_data_l[:6]])}, ..."
        )
        future_l = [
            executor.submit(llm_api_wrapper_partial, data=data) for data in oai_data_l
        ]
        with tqdm(
            total=len(oai_data_l), desc=f"LLM Infer[{args.num_workers}]", ncols=80
        ) as progress:
...
if __name__ == "__main__":
    ...
    args.pop("completion")
    api = OpenAIAPIClient(
        call_url=args.pop("call_url", None), api_key=args.pop("api_key", None)
    )
