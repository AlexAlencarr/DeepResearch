...
from qwen_agent.tools.gpt4o.utils import (
    APIException,
    get_args,
    openai_ret_wrapper,
...
class OpenAIAPIClient(BaseAPIClient):
    def __init__(
        ...
        super(OpenAIAPIClient, self).__init__(time_out=timeout, verbose_num=verbose_num)
    ...
    def call(self, **kwargs):
        ...
        for i in range(self.max_try):
            try:
                ...
                ret_json = ret.json()
                if self.call_url.startswith("http://47.88.8.18") and payload[
                    "model"
                ].startswith("claude"):
                ...
                for output in ret_json["choices"]:
                    if "finish_reason" not in output:
                        raise APIException(f"Error: {ret_json}")
                    if output["finish_reason"].lower() not in [
                        "stop",
                        "function_call",
                        "eos",
                        "end_turn",
...
def test_llm_call(prompt, call_llm="gpt-4o", evaluate=False):
    ...
    if call_url and authorization:
        api = OpenAIAPIClient(call_url=call_url, api_key=authorization, timeout=timeout)
    else:
