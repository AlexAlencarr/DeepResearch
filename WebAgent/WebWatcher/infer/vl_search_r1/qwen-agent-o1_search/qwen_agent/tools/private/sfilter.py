...
def split_sentence_func(text, best=True):
    ...
    _UNDO_AB_SENIOR = re.compile(
        r"([A-Z][a-z]{1,2}\.)" + _SEPARATOR + r"(\w)", re.UNICODE
    )
...
def sfilter(query, ctx, max_retry=20):
    ...
    if "web_main_body" not in ctx or len(ctx.get("web_main_body", "")) <= len(
        ctx["snippet"]
    ):
        return ctx

    all_sentence_list = split_sentence_func(
        ctx["snippet"] + "\n" + ctx["web_main_body"]
    )
    all_sentence = ""
...
输出："""
    ...
    while relevant_sentence_id is None and retries <= max_retry:
        try:
            relevant_sentence_id = requests.request(
    ...
    if relevant_sentence_id == "0":
        ...
        try:
            ...
            split_sentence_id = sorted(
                [int(elem) for elem in relevant_sentence_id.strip().split(" ")]
            )
            split_sentence_text = [
                all_sentence_list[id - 1] for id in split_sentence_id
            ]
