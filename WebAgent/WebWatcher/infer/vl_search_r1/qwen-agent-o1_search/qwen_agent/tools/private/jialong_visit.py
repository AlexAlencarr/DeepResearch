...
from qwen_agent.tools.private.topsdk.client import TopApiClient
from qwen_agent.tools.private.topsdk.defaultability.defaultability import Defaultability
from qwen_agent.tools.private.topsdk.defaultability.request.alibaba_aidata_aignite_application_run_request import (
    AlibabaAidataAigniteApplicationRunAigniteApplicationExecuteReqDTO,
    AlibabaAidataAigniteApplicationRunRequest,
)
...
JUDGE_SUMMARY_MODEL = os.getenv(
    "JUDGE_SUMMARY_NAME", "Qwen2.5-72B-Instruct-SummaryModel-lw-32b"
)
...
class Visit(BaseTool):
    ...
    def aidata_readpage(self, url: str, only_cache=False) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                response_data = json.loads(response["data"]["outputs"])["data"]["data"][
                    0
                ]
    ...
    def readpage(self, url: str, goal: str) -> str:
        ...
        for attempt in range(max_attempts):
            if (
                "wikipedia.org" in url or "wikipedia.com" in url or ""
            ) and attempt <= 1:
                sevice = "self-wiki"
...
if __name__ == "__main__":
    ...
    print(
        visit.readpage(
            "https://www.novartis.com/news/media-releases/novartis-lutathera-significantly-reduced-risk-disease-progression-or-death-72-first-line-treatment-patients-advanced-gastroenteropancreatic-neuroendocrine-tumors",
            "介绍主要内容",
        )
    )
