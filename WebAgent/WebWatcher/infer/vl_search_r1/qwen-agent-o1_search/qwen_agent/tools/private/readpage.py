...
def aidata_readpage(url):
    only_cache = os.getenv("NLP_WEB_SEARCH_ONLY_CACHE", "true").lower() in (
        "y",
        "yes",
        "t",
        "true",
    ...
    alibabaAidataAigniteApplicationRunAigniteApplicationExecuteReqDTO.application_id = (
        2627
    )
    ...
    request.aignite_application_execute_req_dto = (
        alibabaAidataAigniteApplicationRunAigniteApplicationExecuteReqDTO
    )
...
class Visit(BaseTool):
    ...
    async def llm(self, messages):
        ...
        for attempt in range(max_retries):
            ...
            compute_llm_res = await judge_func(
                messages, top_k=-1, json_schema=json_schema
            )
                ...
                messages = [
                    {
                        ...
                        "content": extractor_prompt.format(
                            webpage_content=webpage_content, goal=goal
                        ),
                    }
                ]
