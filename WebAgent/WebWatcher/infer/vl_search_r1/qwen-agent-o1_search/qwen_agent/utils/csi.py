...
server_endpoint = os.getenv(
    "CONTENT_SAFETY_SERVER_ENDPOINT", "epsrv-t4nuxurcnq3orqz22oh2"
)
code = os.getenv(
    "CONTENT_SAFETY_CODE", "bailian_qwen_international_aigc_mtee_sns_unify_check"
)
business_name = os.getenv(
    "CONTENT_SAFETY_BUSINESS_NAME", "aliyun.bailian.international_AIGC"
)
first_product_name = os.getenv(
    "CONTENT_SAFETY_FIRST_PRODUCT_NAME", "qwen_international"
)
    ...
    "argsObjs": [
        code,
        {
            "r": {
                ...
                "operateType": "search",
                "csiUser": {"id": "qwen_chat_websearch"},
                "csiAigc": {
                    "generateStage": "query",
                    "sceneType": "txt2txt",
                    "userInputTexts": [{"content": ""}],
                },
                "ext": {"originalUrl": ""},
            }
        },
...
def csi(text: str, doc_url: str, scene: str = "search") -> str:
    ...
    payload["argsObjs"][1]["r"][
        "srcId"
    ] = f'qwen_chat_{scene}-{get_current_time("%Y-%m-%d")}-{generate_random_string()}'
