...
def parallel_exec(
    ...
    jitter: float = 0.0,
    non_parallelizable_tool_names: List[str] = [
        "real_code_interpreter",
        "code_interpreter",
        "python_executor",
    ],
) -> list:
    ...
    serial_kwargs = [
        ...
        if kwargs.get("tool_name", "") in non_parallelizable_tool_names
    ]
    ...
    list_of_kwargs = [
        kwargs
        for kwargs in list_of_kwargs
        if kwargs.get("tool_name", "") not in non_parallelizable_tool_names
    ]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if len(result) == 3:
                ...
                print(
                    f"[parallel] (rank-{rank}) task {len(results) + 1}/{len(list_of_kwargs)} input -> output {log_kwargs} -> {result}",
                    flush=True,
                )
...
def serial_exec(fn: Callable, list_of_kwargs: List[dict]) -> List[Any]:
    ...
    for idx, kwargs in enumerate(list_of_kwargs):
        ...
        if isinstance(tool_args, dict):
            ...
            tool_args.pop("history_messages", [])
        print(
            f"[serial] (rank-{rank}) task {idx + 1}/{len(list_of_kwargs)} input {log_kwargs}",
            flush=True,
        )
        result = fn(**kwargs)
        print(
            f"[serial] (rank-{rank}) task {idx + 1}/{len(list_of_kwargs)} input -> output {log_kwargs} -> {result}",
            flush=True,
        )
        results.append(result)
    return results
