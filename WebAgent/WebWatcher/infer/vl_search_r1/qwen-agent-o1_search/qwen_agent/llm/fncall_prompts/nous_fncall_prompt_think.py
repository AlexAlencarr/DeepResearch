...
THINKING_MODE = True
SPECIAL_CODE_MODE = os.getenv("SPECIAL_CODE_MODE", "true").lower() == "true"
CODE_TOOL_PATTERN = "code_interpreter"


class NousFnCallPromptThink(BaseFnCallPrompt):
                ...
                if THINKING_MODE:
                    ...
                    for c in content:
                        if c.text and "<think>" in c.text:
                            c.text = re.sub(
                                r"<think>.*?</think>", "", c.text, flags=re.DOTALL
                            ).strip()
                            if c.text:
        ...
        for msg in messages:
            ...
            for item in content:
                ...
                if THINKING_MODE:
                    ...
                    new_content.append(
                        ContentItem(text="</think>".join(_item_text[:-1]) + "</think>")
                    )
                ...
                for txt in tool_call_list[1:]:
                    ...
                    if (
                        SPECIAL_CODE_MODE
                        and "<code>" in one_tool_call_txt[0]
                        and "</code>" in one_tool_call_txt[0]
                    ):
