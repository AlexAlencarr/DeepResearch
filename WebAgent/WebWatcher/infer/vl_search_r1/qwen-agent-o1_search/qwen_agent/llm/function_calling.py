...
class BaseFnCallModel(BaseChatModel, ABC):
    ...
    def __init__(self, cfg: Optional[Dict] = None):
        ...
        if fncall_prompt_type == "qwen":
            ...
            from qwen_agent.llm.fncall_prompts.nous_fncall_prompt_think import (
                NousFnCallPromptThink,
            )

            self.fncall_prompt = NousFnCallPromptThink()
        elif fncall_prompt_type == "code":
            from qwen_agent.llm.fncall_prompts.code_fncall_prompt import (
                FN_STOP_WORDS,
                CodeFnCallPrompt,
            )

            self.fncall_prompt = CodeFnCallPrompt()
            stop = self.generate_cfg.get("stop", [])
            self.generate_cfg["stop"] = stop + [
                x for x in FN_STOP_WORDS if x not in stop
            ]
    ...
    def _chat_with_functions(
        ...
        for k in ["parallel_function_calls", "function_choice"]:
            if k in generate_cfg:
                del generate_cfg[k]
        return self.continue_assistant_response(
