...
class OmniSearch:
    def __init__(self, base_url="http://0.0.0.0:8001/v1", api_key="EMPTY"):
        ...
        self.qwen_agent = Qwen_agent(
            function_list=["web_search", "VLSearchImage", "visit", "code_interpreter"]
        )
        ...
        messages = [
            ...
            dict(
                ...
                content=[
                    {
                        "type": "text",
                        "text": prompt_ins.replace(
                            "{Image_url}", sample["file_path"]
                        ).replace("{Question}", sample["prompt"]),
                    },
                    {"type": "image_url", "image_url": {"url": test_img_base64}},
                ],
            ),
        ]
            ...
            if action is None:
                ...
                try:
                    ...
                    if request_para is None:
                        raise Exception(
                            f"Invalid request parameters. request_para is None, content:{content}"
                        )

                    img_save_path = (
                        "scripts_eval/scripts_eval/images/search_image/"
                ...
                if request_para is None:
                    ...
                    if len(images_path) > 0:
                        ...
                        user_content.append({"type": "text", "text": search_results})
                    else:
                        user_content.append({"type": "text", "text": search_results})
        ...
        for attempt in range(max_retry + 1):
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    print(
                        f"[Timeout] Inference timeout for sample {sample.get('file_path','N/A')}, retry {attempt+1}/{max_retry}"
                    )
                except Exception as e:
                    print(
                        f"[Exception] Inference error for sample {sample.get('file_path', 'N/A')}: {e}, retry {attempt+1}/{max_retry}"
                    )
        print(
            f"[Fail] Inference failed after {max_retry+1} attempts for sample {sample.get('file_path','N/A')}"
        )
    ...
    def eval(self, input_file_list, output_file):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_index = {
                executor.submit(self.infer_with_timeout_retry, sample): idx
