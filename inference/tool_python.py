...
class PythonInterpreter(BaseToolWithFileAccess):
    ...
    def observation(
        ...
        readpage: bool = False,
        max_observation_length: int = None,
        tokenizer=None,
    ):
        print("test")
        assert isinstance(
            tool_results, str
        ), f"result of python code should be str, instead of {type(tool_results)}. {tool_results}"
        return tool_results
            ...
            for attempt in range(8):
                try:
                    ...
                    code_result = run_code(
                        RunCodeRequest(
                            code=code, language="python", run_timeout=timeout
                        ),
                        max_attempts=1,
                    ...
                    if code_result.run_result.execution_time >= timeout - 1:
                        result.append(
                            f"[PythonInterpreter Error] TimeoutError: Execution timed out."
                        )
                    result = "\n".join(result)
                    ...
                    last_error = (
                        f"[Python Interpreter Error]: {str(e)} on endpoint {endpoint}"
                    )
    ...
    def call_specific_endpoint(
        ...
        **kwargs,
        ...
        try:
            ...
            return (
                True,
                result if result.strip() else "Finished execution.",
                execution_time,
            )
            ...
            return (
                False,
                f"[Python Interpreter Error] TimeoutError: Execution timed out.",
                None,
            )
        except Exception as e:
