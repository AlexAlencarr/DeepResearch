...
class VideoAnalysis(BaseTool):
    ...
    def _init_config(self, cfg: Dict) -> Dict:
        ...
        return {
            ...
            or os.getenv("VIDEO_MODEL_NAME", "qwen-omni-turbo"),
            "analysis_model": cfg.get("analysis_model")
            or os.getenv("VIDEO_ANALYSIS_MODEL_NAME", "qwen-plus-latest"),
    ...
    def _check_dependencies(self):
        ...
        try:
            ...
            logger.warning(
                "ffmpeg-python not installed. Will use subprocess fallback for media operations."
            )
            self.ffmpeg = None
        ...
        try:
            ...
            with temp_directory() as temp_dir:
                ...
                audio_path = (
                    media_path
                    if is_audio
                    else self._extract_audio(media_path, temp_dir)
                )
                ...
                if not is_audio:
                    frames = self._extract_keyframes(
                        media_path, min(params["num_frames"], self.config["max_frames"])
                    )
    ...
    def _parse_params(self, params: Union[str, Dict]) -> Dict:
        ...
        return {
            ...
            "prompt": params["prompt"],
            "num_frames": min(
                int(params.get("num_frames", DEFAULT_FRAMES)), self.config["max_frames"]
            ),
        }
    ...
    def _is_supported_type(self, extension: str) -> bool:
        ...
        return ext in {
            ext.lstrip(".") for ext in SUPPORTED_VIDEO_TYPES | SUPPORTED_AUDIO_TYPES
        }
        ...
        if self.ffmpeg:
            try:
                (
                    ...
                    .output(
                        str(output_path), vn=None, acodec="libmp3lame", loglevel="error"
                    )
                ...
                logger.warning(
                    f"ffmpeg-python extraction failed: {str(e)}. Trying subprocess fallback."
                )
    ...
    def _transcribe_audio(self, audio_path: Path) -> str:
        ...
        try:
            ...
            messages = [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "Completely transcribe this audio content with all details",
                        },
                        {
                            "type": "input_audio",
                            "input_audio": {
            ...
            response = self.client.chat.completions.create(
                model=self.config["video_model"], messages=messages, stream=True
            )
            ...
            logger.info(
                f"Transcription completed (time: {time.time() - start_time:.1f}s, chars: {len(final_text)})"
            )
            return final_text
    ...
    def _extract_keyframes(self, video_path: Path, num_frames: int) -> List[str]:
        ...
        try:
            ...
            if self._scene_detect_available:
                frames = self._extract_frames_with_scene_detection(
    ...
    def _extract_frames_with_scene_detection(
        ...
        try:
            ...
            if not timestamps or len(timestamps) < num_frames:
                ...
                if additional_needed > 0:
                    ...
                    extra_timestamps = [
                        interval * (i + 1) for i in range(additional_needed)
                    ]
            ...
            logger.warning(
                f"Scene detection failed, falling back to uniform sampling: {str(e)}"
            )
    ...
    def _analyze_media(
        self, prompt: str, transcript: str, frames: List[str], is_audio: bool
    ) -> str:
