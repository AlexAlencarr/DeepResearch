...
class IDP:
    ...
    def file_parser_query(self, fid):
        request = docmind_api20220711_models.QueryDocParserStatusRequest(id=fid)
        try:
        ...
        responses = dict()
        for i in range(
            0, NumberOfSuccessfulParsing["number_of_successful_parsing"], 3000
        ):
            request = docmind_api20220711_models.GetDocParserResultRequest(
                id=fid, layout_step_size=3000, layout_num=i
            )
            try:
